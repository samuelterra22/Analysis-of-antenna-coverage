...
// km - 050520 lPitchSz was not initialized and caused crashes
#ifdef BUGFIX_03
	INT32 lPitchSz = 0;
#else
	INT32 lPitchSz;
...
// km - 050513
#ifdef BUGFIX_04
		vtp_extract(sPitchAv, plPitch, lPitchSz, psNormSpeech, lNormSpeechSz, ppdVtp, &lNumFrames );
#else
...
FLOAT FFT_gaps(const INT16 *speech, const INT32 speech_Nelements, const INT32 *final_pitch, const INT32 final_pitch_Nelements)
{
	...
	// km - 050513
	// type conversion from int to unsigned int leads to illegal value
#ifdef BUGFIX_05
	for (pos_pitch=1; (INT32)(pos_pitch)<(INT32)(final_pitch_Nelements-1); pos_pitch++) 
#else
	for (pos_pitch=1; pos_pitch<(UINT32)(final_pitch_Nelements-1); pos_pitch++)
#endif
	{
		pos_temp = (UINT32) round(( ((FLOAT) final_pitch[pos_pitch]) / HALF_FFT_FRAME_SIZE) );
		...
		spec_unit = (INT32) round((FFT_FRAME_SIZE / (FLOAT) pitch_length));
		half_spec_unit = round( (FLOAT) spec_unit /2);
		quarter_spec_unit = round( (FLOAT) spec_unit /4);
...
// km - 050930
#ifdef BUGFIX_10	// division by zero
		if (VTP_av[cpt+1] == 0)
				VTP_av[cpt+1] = 1;
#endif
...
// km - 050930
#ifdef BUGFIX_11	// division by zero
		correl_offset_Nelements = max(correl_offset_Nelements,1);
#endif
...
FLOAT VTPPeakTracker(FLOAT **VTP, INT32 VTP_Nelements, FLOAT *fpPeakDiffOut, FLOAT *fpFinalVtpOut)
{
	...
	*fpPeakDiffOut = 0;
	...
	// km - 050520
	// divisions by zero have occured
#ifdef BUGFIX_06
	if((VTP_Nelements-k)> 0)
...
UINT32 mute_spotter(const INT16 *psRawSpeech, const INT32 psRawSpeechSz)
{
	...
	for (cnt=0;cnt<psRawSpeechSz;cnt++) Sspeech1[cnt] = (FLOAT)round(Fspeech[cnt]);
